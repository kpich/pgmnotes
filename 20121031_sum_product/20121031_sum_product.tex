...
\node[state]         (A) {$\Omega\ \ \ \theta\ \ \ \theta'$};
  \node[state] (B)     [right of=A] {$\mu\ \ \ \ \Mu\ \ \ \mu'$};
...
$A\theta=\sup_{\mu\in \Mu}\{\left<\theta,\mu\right>-A^*(\mu)\}$\\
...
If $\mu\in \Mu(G)$ it satisfies norm and marginal constraints $\rightarrow \mu \in \mathds{L}G$\\
There are exponential constraints for $\mu$ and linear constraints for $\mathds{L}(G)$\\
$$
\begin{array}{ll}
P(x_1,\dots, x_n)
\mu_i(x_i)&=P(x_i)\\
